...
class UWG(object):
    ...
    def __init__(self, epwFileName,month_name, uwgParamFileName=None, epwDir=None, uwgParamDir=None, destinationDir=None, destinationFileName=None):
    ...
    def simulate(self):
        ...
        for it in range(1,self.simTime.nt,1):# for every simulation time-step defined by uwg
              ...
              if it == Output_TimInd_new[iO_new]:
                  ...
                  coolConsump_hourly[iO_new] = self.BEM[0].building.coolConsump
                  sensHeatDemand_hourly[iO_new] = self.BEM[0].building.sensHeatDemand
                  heatConsump_hourly[iO_new] = self.BEM[0].building.heatConsump
                  waterHeatConsump_hourly[iO_new] = self.BEM[0].building.waterHeatConsump
                  Q_st_hourly[iO_new] = self.BEM[0].building.Q_st
                  Q_he_st_hourly[iO_new] = self.BEM[0].building.Q_he_st
                  Q_bites_hourly[iO_new] = self.BEM[0].building.Q_bites
                  Q_hp_hourly[iO_new] = self.BEM[0].building.Q_hp
                  Q_recovery_hourly[iO_new] = self.BEM[0].building.Q_recovery
                  W_hp_hourly[iO_new] = self.BEM[0].building.W_hp
                  W_pv_hourly[iO_new] = self.BEM[0].building.W_pv
                  COP_hp_hourly[iO_new] = self.BEM[0].building.COP_hp
                  indoorTemp_hourly[iO_new] = self.BEM[0].building.indoorTemp
                  T_st_f_i_hourly[iO_new] = self.BEM[0].building.T_st_f_i
                  T_st_f_o_hourly[iO_new] = self.BEM[0].building.T_st_f_o
                  T_he_st_i_hourly[iO_new] = self.BEM[0].building.T_he_st_i
                  T_he_st_o_hourly[iO_new] = self.BEM[0].building.T_he_st_o
                  T_bites_hourly[iO_new] = self.BEM[0].building.T_bites
                  W_wt_hourly[iO_new] = self.BEM[0].building.W_wt
                  f_pcm_hourly[iO_new] = self.BEM[0].building.f_pcm
                  Q_waterSaved_hourly[iO_new] = self.BEM[0].building.Q_waterSaved
                  sensWaterHeatDemand_hourly[iO_new] = self.BEM[0].building.sensWaterHeatDemand
                  Q_ground_hourly[iO_new] = self.BEM[0].building.Q_ground
                  elecDomesticDemand_hourly[iO_new] = self.BEM[0].building.elecDomesticDemand
                  Q_waterRecovery_hourly[iO_new] = self.BEM[0].building.Q_waterRecovery
                  TCanyon_hourly[iO_new] = self.BEM[0].building.Tcanyon
                  #infil_hourly[iO_new] = self.BEM[0].building.infil
                  vent_hourly[iO_new] = self.BEM[0].building.vent
        ...
        outputFile_BEM.write("# 0:time [hr] 1:sensWaste (Total Sensible waste heat per unit building footprint area including cool heat dehum water and gas) [W m^-2] 2:dehumDemand (Latent heat demand for dehumidification of air per unit building footprint area) [W m^-2] 3:QWater (Water heating waste heat) [W m^-2] 4:QGas [W m^-2] "
                             ...
                             "31: Canyon Temperature [K] 32: Infiltration Rate (includes windows) [ACH] or [hr^-1] 33: Ventilation Rate (forced air) [m^3 s^-1 m^-2]  34: HVAC Flag (0 = Fully Conditioned, 1 = Natural Ventilation and Conditioned) 35: Window State (0 = closed 1 = open) \n")
        for i in range(0,len(Output_TimInd_new)):
            outputFile_BEM.write("%i %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %i %f \n"
