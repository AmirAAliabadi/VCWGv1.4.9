...
class Montreal():
    def __init__(self, City, BaseCaseName, CaseName, Scenario,
        ...
        QC_Residential_Electricity_Rates = pd.read_csv('resources/Economics/QC_Residential_Electricity_Rates.txt',delimiter=',')

        # Gas price rates
        QC_NG_Residential_Rates = pd.read_csv('resources/Economics/QC_NG_Residential_Rates.txt', delimiter=',')
        ...
        #Provincial
        #Renoclimat Program
        ProvRebRenoIsulation = 2000         #[$] 35-2500$
        ProvRebRenoAirSealing = 245         #[$] 245-490$
        ProvRebRenoWindowsDoors = 300       #[$] 60 per rough opening in the wall or roof
        ...
        RvalWallBaseCase = 4.05 # [m2 K W-1]
        RvalRoofBaseCase = 5.46 # [m2 K W-1]
        ...
        CIniRetrofit = ((PVSwitch * A_pv * PVPrice + WTSwitch * A_wt * WTPrice + STSwitch * A_st * STPrice + BITESSwitch * V_bites * BITESPrice + \
                        ...
                        EnvSwitch * ProvRebRenoIsulation + AirTSwitch * ProvRebRenoAirSealing + HPSwitch * ProvRebHP +\
                        WindowDoorReplaceSwitch + ProvRebRenoWindowsDoors))
        ...
        for year in range(1, Nyears + 1):
            for month_idx, month in enumerate(months, start=0):
                ...
                if os.path.exists(FilePathRetrofitMonthly) and os.path.exists(FilePathBaseMonthly):
                    ...
                    Gas = PerfMetricsMonthlyBase[1] + PerfMetricsMonthlyBase[6]
                    ...
                    PresBaseGasCost = PresBaseGasCost + (((PerfMetricsMonthlyBase[1] + PerfMetricsMonthlyBase[6]) * \
                      (QC_NG_Residential_Rates.iloc[0, 0] + QC_NG_Residential_Rates.iloc[0, 1] + QC_NG_Residential_Rates.iloc[0, 2] + \
                        QC_NG_Residential_Rates.iloc[0, 3] + QC_NG_Residential_Rates.iloc[0, 4] + QC_NG_Residential_Rates.iloc[0, 5])/100) * A_building) * \
                          (1 + GasPriceInc) ** year * 1 / ((1 + EffIntRate) ** year)

                    PresRetrofitGasCost = PresRetrofitGasCost + (((PerfMetricsMonthlyRetrofit[1] + \
                      PerfMetricsMonthlyRetrofit[6]) * (QC_NG_Residential_Rates.iloc[0, 0] + QC_NG_Residential_Rates.iloc[0, 1] + \
                        QC_NG_Residential_Rates.iloc[0, 2] + QC_NG_Residential_Rates.iloc[0, 3] + QC_NG_Residential_Rates.iloc[0, 4] + \
                           QC_NG_Residential_Rates.iloc[0, 5])/100) * A_building) * (1 + GasPriceInc) ** year * 1 / ((1 + EffIntRate) ** year)
                    ...
                    if MonthlyElecDemandBase <= QC_Step:
                        PresBaseElecCost = PresBaseElecCost + (((MonthlyElecDemandBase) * QC_Residential_Electricity_Rates.iloc[0, 0]/100) * \
                                    A_building) * (1 + ElecPriceInc) ** year * 1 / ((1 + EffIntRate) ** year)

                    if MonthlyElecDemandBase >= QC_Step:
                        PresBaseElecCost = PresBaseElecCost + (((MonthlyElecDemandBase) * QC_Residential_Electricity_Rates.iloc[0, 1] / 100) * \
                                    A_building) * (1 + ElecPriceInc) ** year * 1 / ((1 + EffIntRate) ** year)

                    if MonthlyElecDemandRetrofit <= QC_Step:
                        PresRetrofitElecCost = PresRetrofitElecCost + (((MonthlyElecDemandRetrofit) * QC_Residential_Electricity_Rates.iloc[0, 0] / 100) * \
                                    A_building) * (1 + ElecPriceInc) ** year * 1 / ((1 + EffIntRate) ** year)

                    if MonthlyElecDemandRetrofit >= QC_Step:
                        PresRetrofitElecCost = PresRetrofitElecCost + (((MonthlyElecDemandRetrofit) * QC_Residential_Electricity_Rates.iloc[0, 1] / 100) * \
                                    A_building) * (1 + ElecPriceInc) ** year * 1 / ((1 + EffIntRate) ** year)
                else:
