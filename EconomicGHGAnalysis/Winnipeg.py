...
class Winnipeg():
    def __init__(self, City, BaseCaseName, CaseName, Scenario,
        ...
        MB_Residential_Electricity_Rates = pd.read_csv('resources/Economics/MB_Residential_Electricity_Rates.txt',
                                                       delimiter=',')

        # Gas price rates
        MB_NG_Residential_Rates = pd.read_csv('resources/Economics/MB_NG_Residential_Rates.txt', delimiter=',')
        ...
        #15years
        ProvLoanHEELPV = 20000                     # [$]
        ProvLoanHEELHP = 10000                     # [$]
        ProvLoanHEELPVHPNyears = 15                # [years]


        #5years
        #Eligible measures (windows, doors, insulation, etc.)
        ProvLoanHEELWindowsDoorsInsulation = 7500      # [$]
        ProvLoanHEELWindowsDoorsInsulationNyears = 5   # [years]

        ProvLoanHEELNGFurnace = 5500               # [$]
        ProvLoanHEELNGBoilers = 5500               # [$]

        #HEEl ANNUAL INTEREST RATE
        #Interest rate now is 0.066 but it should be fraction of the prime rate
        ProvLoanHEELInt = 0.0318                #Actual intrate is 0.066 when National Bank of Canada interst rate is 0.0784

        #SOLAR REBATE PROGRAM 5000$
        ProvRebPV = 5000                           # [$]

        if (Nyears < FedLoanNyearsForgiveness) or (Nyears < ProvLoanHEELPVHPNyears) or (
                Nyears < ProvLoanHEELWindowsDoorsInsulationNyears):
        ...
        CIniRetrofit = (PVSwitch * A_pv * PVPrice + WTSwitch * A_wt * WTPrice + STSwitch * A_st * STPrice + \
                       ...
                       AirTSwitch * AirTPrice + TreePrice * Additional_Trees - (FedLoanCanGreener + PVSwitch * ProvLoanHEELPV +\
                        + HPSwitch * ProvLoanHEELHP + WindowDoorReplaceSwitch * ProvLoanHEELWindowsDoorsInsulation + PVSwitch * FedRebPV +\
                        PVSwitch * ProvRebPV + AirTSwitch * FedRebAirSealing + EnvSwitch * FedRebInsulation +\
        ...
        PresProvloanHEELHPPayment, PresFedLoanPayment, PresProvLoanHEELPVPayment, PresProvloanHEELWindowsDorrsInsultionPayment ,\
            FuelCO2Saving, ElecCO2Saving,TotalCO2Sav,PresSCCSav = [0] * 26
        Payback = []
        CO2Saving = []
        ...
        for year in range(1, Nyears + 1):
            for month_idx, month in enumerate(months, start=0):
                ...
                if os.path.exists(FilePathRetrofitMonthly) and os.path.exists(FilePathBaseMonthly):
                    ...
                    PresBaseGasCost = PresBaseGasCost + (MB_NG_Residential_Rates.iloc[0, 0] + ((PerfMetricsMonthlyBase[1] + \
                         PerfMetricsMonthlyBase[6]) * ( MB_NG_Residential_Rates.iloc[0, 1] + MB_NG_Residential_Rates.iloc[0, 2]))* \
                                A_building) * (1 + GasPriceInc) ** year * 1 / ((1 + EffIntRate) ** year)

                    PresRetrofitGasCost = PresRetrofitGasCost + (MB_NG_Residential_Rates.iloc[0, 0] + \
                       ((PerfMetricsMonthlyRetrofit[1] + PerfMetricsMonthlyRetrofit[6]) * (MB_NG_Residential_Rates.iloc[0, 1] + \
                          MB_NG_Residential_Rates.iloc[0, 2])) * A_building) * (1 + GasPriceInc) ** year * 1 / ((1 + EffIntRate) ** year)
                    ...
                    # Check step consumption and consider unit conversion from Amp-h = (1000/120) kW-h
                    if MonthlyElecDemandBase * (1000/120) < MB_Residential_Electricity_Rates.iloc[0,0]:
                        PresBaseElecCost = PresBaseElecCost + (MB_Residential_Electricity_Rates.iloc[0,1] + \
                            ((MonthlyElecDemandBase) * MB_Residential_Electricity_Rates.iloc[0, 3] / 100) * A_building) * \
                                           (1 + ElecPriceInc) ** year * 1 / ((1 + EffIntRate) ** year)
                    elif MonthlyElecDemandBase * (1000/120) >= MB_Residential_Electricity_Rates.iloc[0,0]:
                        PresBaseElecCost = PresBaseElecCost + (MB_Residential_Electricity_Rates.iloc[0, 2] + \
                            ((MonthlyElecDemandBase) * MB_Residential_Electricity_Rates.iloc[0, 3] / 100) * A_building) * \
                                       (1 + ElecPriceInc) ** year * 1 / ((1 + EffIntRate) ** year)

                    if MonthlyElecDemandRetrofit * (1000 / 120) < MB_Residential_Electricity_Rates.iloc[0, 0]:
                        PresRetrofitElecCost = PresRetrofitElecCost + (MB_Residential_Electricity_Rates.iloc[0, 1] + \
                            ((MonthlyElecDemandRetrofit) * MB_Residential_Electricity_Rates.iloc[0, 3] / 100) * A_building) * \
                                           (1 + ElecPriceInc) ** year * 1 / ((1 + EffIntRate) ** year)
                    elif MonthlyElecDemandRetrofit * (1000 / 120) >= MB_Residential_Electricity_Rates.iloc[0, 0]:
                        PresRetrofitElecCost = PresRetrofitElecCost + (MB_Residential_Electricity_Rates.iloc[0, 2] + \
                            ((MonthlyElecDemandRetrofit) * MB_Residential_Electricity_Rates.iloc[0, 3] / 100) * A_building) * \
                                           (1 + ElecPriceInc) ** year * 1 / ((1 + EffIntRate) ** year)
                else:
            ...
            #Home Energy Efficiency Loan (HEEl)Payment
            if year <= ProvLoanHEELPVHPNyears:
                PresProvLoanHEELPVPayment = PresProvLoanHEELPVPayment + (ProvLoanHEELPV * (ProvLoanHEELInt * (1 + ProvLoanHEELInt) **\
                    ProvLoanHEELPVHPNyears) / ((1 + ProvLoanHEELInt) ** ProvLoanHEELPVHPNyears - 1)) *\
                           (1 / ((1 + EffIntRate) ** year))

                PresProvloanHEELHPPayment = PresProvloanHEELHPPayment + (ProvLoanHEELHP * (ProvLoanHEELInt * (1 + ProvLoanHEELInt) **\
                    ProvLoanHEELPVHPNyears) / ((1 + ProvLoanHEELInt) ** ProvLoanHEELPVHPNyears - 1)) * \
                                (1 / ((1 + EffIntRate) ** year))

            if year <= ProvLoanHEELWindowsDoorsInsulationNyears:
                PresProvloanHEELWindowsDorrsInsultionPayment = PresProvloanHEELWindowsDorrsInsultionPayment + (ProvLoanHEELWindowsDoorsInsulation * (ProvLoanHEELInt *\
                (1 + ProvLoanHEELInt) ** ProvLoanHEELWindowsDoorsInsulationNyears) / ((1 + ProvLoanHEELInt) ** ProvLoanHEELWindowsDoorsInsulationNyears - 1)) *\
                           (1 / ((1 + EffIntRate) ** year))
            ...
            SumAnnualCostDiff = PresFedLoanPayment + PVSwitch * PresProvLoanHEELPVPayment + HPSwitch * PresProvloanHEELHPPayment +\
                    WindowDoorReplaceSwitch * PresProvloanHEELWindowsDorrsInsultionPayment + PresRetrofitGasCost + PresRetrofitElecCost +\
        ...
        #Total Loan Payment
        PresLoanPayment = PresFedLoanPayment + PVSwitch * PresProvLoanHEELPVPayment + HPSwitch * PresProvloanHEELHPPayment +\
                          WindowDoorReplaceSwitch * PresProvloanHEELWindowsDorrsInsultionPayment
