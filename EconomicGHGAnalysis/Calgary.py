...
class Calgary():
    def __init__(self, City, BaseCaseName, CaseName, Scenario,
        ...
        # Gas price rates
        AB_NG_Residential_Rates = pd.read_csv('resources/Economics/AB_NG_Residential_Rates.txt', delimiter=',')
        ...
        ProvLoanCEIP = 3000                  #[$]
        ProvLoanCEIPIntRate = 0.0142         #Actual intrate is 0.0295 when National Bank of Canada interst rate is 0.0784
        ProvLoanNyears = 20                  # [years]

        if (Nyears < FedLoanNyearsForgiveness) or (Nyears < ProvLoanNyears):
        ...
        CIniRetrofit = (PVSwitch * A_pv * PVPrice + WTSwitch * A_wt * WTPrice + STSwitch * A_st * STPrice + BITESSwitch * V_bites * BITESPrice + PCMSwitch * V_pcm * PCMPrice + \
                CRSwitch * A_CR * CRPrice + HPSwitch * HPPrice) * A_building + EnvSwitch * EnvPrice + AirTSwitch * AirTPrice + TreePrice * Additional_Trees - \
                        (FedLoanCanGreener + ProvLoanCEIP + PVSwitch * FedRebPV + AirTSwitch * FedRebAirSealing + EnvSwitch * FedRebInsulation + WindowDoorReplaceSwitch * FedRebWindowsDoors)
        ...
        PresFedLoanPayment, PresProvloanCEIPPayment,FuelCO2Saving, ElecCO2Saving,TotalCO2Sav,PresSCCSav = [0] * 24
        ...
        for year in range(1, Nyears + 1):
            for month_idx, month in enumerate(months, start=0):
                ...
                if os.path.exists(FilePathRetrofitMonthly) and os.path.exists(FilePathBaseMonthly):
                    ...
                    PresBaseGasCost = PresBaseGasCost + (((PerfMetricsMonthlyBase[1] + PerfMetricsMonthlyBase[6]) * \
                        AB_NG_Residential_Rates.iloc[month_idx, 1] / GJtoM3) * A_building) * (1 + GasPriceInc) **\
                                      year * 1 / ((1 + EffIntRate) ** year)

                    PresRetrofitGasCost = PresRetrofitGasCost + (((PerfMetricsMonthlyRetrofit[1] +PerfMetricsMonthlyRetrofit[6]) *\
                        AB_NG_Residential_Rates.iloc[month_idx, 1] / GJtoM3) * A_building) * (1 + GasPriceInc) **\
                                          year * 1 / ((1 + EffIntRate) ** year)

                    # Electricity Consumption Cost
                    # ENMAX Energy is the provider for electricity in Calgary
                    ...
                    PresBaseElecCost = PresBaseElecCost + (((MonthlyElecDemandBase) * AB_Residential_Electricity_Rates.iloc[
                         month_idx, 3] / 100).sum() * A_building) * (1 + ElecPriceInc) ** year *1 / ( (1 + EffIntRate) ** year)

                    PresRetrofitElecCost = PresRetrofitElecCost + (((MonthlyElecDemandRetrofit) *AB_Residential_Electricity_Rates.iloc[
                         month_idx, 3] / 100).sum() *  A_building) * (1 + ElecPriceInc) ** year * 1 / ((1 + EffIntRate) ** year)
            ...
            # Calculate the payment of Clean Energy Improvement Program(CEIP) with 2.95% interest rate over 20 years
            if year <= ProvLoanNyears:
                PresProvloanCEIPPayment = PresProvloanCEIPPayment + (ProvLoanCEIP * (ProvLoanCEIPIntRate * (1 + ProvLoanCEIPIntRate) ** ProvLoanNyears) /\
                    ((1 + ProvLoanCEIPIntRate) ** ProvLoanNyears - 1)) * (1 / ((1 + EffIntRate) ** year))
            ...
            SumAnnualCostDiff = PresFedLoanPayment + PresProvloanCEIPPayment + PresRetrofitGasCost + PresRetrofitElecCost +\
        ...
        #Total Loan Payment
        PresLoanPayment = PresFedLoanPayment + PresProvloanCEIPPayment
