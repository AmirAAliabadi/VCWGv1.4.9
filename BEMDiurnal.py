...
def BEMDiurnal(Adv_ene_heat_mode, outputFileName):
    ...
    df = pd.read_csv('Output/BEM_hourly.txt', delimiter=' ', skiprows=SpinUpHours+3, header=None)
    ...
    # 7: sensHeatDemand [W m^-2]
    # 5: sensCoolDemand [W m^-2]
    # 6: coolConsump [W m^-2]
    # 8: heatConsump [W m^-2]
    # 9: waterHeatConsump [W m^-2]
    # 13: Q_hp [W m^-2]
    # 15: W_hp [W m^-2]
    # 27: sensWaterHeatDemand [W m^-2]
    # 16: W_pv [W m^-2]
    # 24: W_wt [W m^-2]
    # 26: Q_waterSaved [W m^-2]
    # 29: elecDomesticDemand [W m^-2]
    ...
    if Adv_ene_heat_mode == 2:
        DiurnalSensHeatDemand = df[df.columns[7]].groupby(df['remainder_to_24']).sum().values / 1000
        DiurnalGasConsumpHeat = (df[df.columns[8]].groupby(df['remainder_to_24']).sum().values / 1000) * 3600 / HeatingValue
        TotalElecHeatDemand = 0

        DiurnalSensCoolDemand = df[df.columns[5]].groupby(df['remainder_to_24']).sum().values/1000
        DiurnalElecCoolDemand = df[df.columns[6]].groupby(df['remainder_to_24']).sum().values/1000
        ...
        DiurnalSensWaterHeatDemand = df[df.columns[27]].groupby(df['remainder_to_24']).sum().values/1000
        DiurnalGasConsumpWaterHeat = (df[df.columns[9]].groupby(df['remainder_to_24']).sum().values/1000) * 3600 / HeatingValue
    ...
    if Adv_ene_heat_mode == 3:
        ...
        DiurnalElecProducedPV= df[df.columns[16]].groupby(df['remainder_to_24']).sum().values/1000
        DiurnalElecProducedWT= df[df.columns[24]].groupby(df['remainder_to_24']).sum().values/1000
        DiurnalElecDomesticDemand= df[df.columns[29]].groupby(df['remainder_to_24']).sum().values/1000

    # Renewable energy system under heating mode
    if Adv_ene_heat_mode == 1:
        DiurnalSensHeatDemand = df[df.columns[7]].groupby(df['remainder_to_24']).sum().values / 1000 +\
                                df[df.columns[13]].groupby(df['remainder_to_24']).sum().values / 1000
        DiurnalGasConsumpHeat = (df[df.columns[8]].groupby(df['remainder_to_24']).sum().values / 1000) * 3600 / HeatingValue
        DiurnalElecHeatDemand = df[df.columns[15]].groupby(df['remainder_to_24']).sum().values / 1000
        ...
        # so to calculate Diurnal building sensible water heating demand they must be added
        DiurnalSensWaterHeatDemand = df[df.columns[27]].groupby(df['remainder_to_24']).sum().values / 1000 + \
                                     df[df.columns[26]].groupby(df['remainder_to_24']).sum().values / 1000
    ...
    # Renewable energy system under cooling mode
        ...
        DiurnalSensCoolDemand = df[df.columns[5]].groupby(df['remainder_to_24']).sum().values / 1000 + \
                                df[df.columns[13]].groupby(df['remainder_to_24']).sum().values / 1000
        DiurnalElecCoolDemand = df[df.columns[6]].groupby(df['remainder_to_24']).sum().values / 1000 + \
                                df[df.columns[15]].groupby(df['remainder_to_24']).sum().values / 1000
